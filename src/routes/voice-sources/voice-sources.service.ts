...
class VoiceSourcesService {
  async createVoiceSource(
    ...
    const createdVoiceSource =
      await voiceSourcesRepository.createVoiceSource(voiceText);
  ...
  async getVoiceSources(): Promise<
    ApiResponse<CreateVoiceSourceResponseDto[]>
  > {
  ...
  async getVoiceSourceById(
    voiceId: string,
  ): Promise<ApiResponse<CreateVoiceSourceResponseDto>> {
    const voiceSource =
      await voiceSourcesRepository.getVoiceSourceById(voiceId);
